...
type BriefResponsePayload = Omit<BriefPreview, "content"> & {
  generatedContent?: BriefPreview["content"];
  content?: BriefPreview["content"];
};
...
const questionProviderMessages = {
  gemini: "Specialized questions generated by VenturePack's Adaptive Checklist engine.",
  rules: "Questions generated from VenturePack's default preparation checklist.",
} as const;
const briefProviderMessages = {
  gemini: "Drafted by VenturePack's preparation brief engine using your saved answers.",
  rules: "Generated from your saved checklist answers.",
} as const;
...
export function AdaptiveChecklistClient({ requestedBriefType }: { requestedBriefType?: ChecklistBriefType }) {
  ...
  const [questionProviderMessage, setQuestionProviderMessage] = useState("");
  ...
  const [briefProviderMessage, setBriefProviderMessage] = useState("");
  ...
  async function generateBrief(briefType: ChecklistBriefType) {
    ...
    try {
      ...
      setBriefPreview({
        ...payload.brief,
        content: payload.brief.generatedContent ?? payload.brief.content ?? {
          sections: [],
          warnings: [],
          disclaimer: "",
        },
      });
      setBriefProviderMessage(briefProviderMessages[payload.provider ?? "rules"]);
  ...
  async function generateQuestions() {
    ...
    try {
      ...
      setQuestionProviderMessage(questionProviderMessages[payload.provider ?? "rules"]);
      setAnswerDrafts({});
      setSavedQuestionId("");
      await refreshProgress();
    } catch {
        ...
        {briefPreview ? (
          <div className="mt-5 grid gap-5">
            <div className="rounded-2xl border border-[#DCE7F3] bg-[#F8FAFC] p-4">
              ...
              {briefProviderMessage ? (
                <p className="mt-4 rounded-xl border border-[rgba(0,158,167,0.22)] bg-white p-4 text-sm font-semibold leading-6 text-[#00173C]">
                  {briefProviderMessage}
                </p>
              ) : null}
      ...
      <section className="rounded-3xl border border-[#DCE7F3] bg-white p-5 shadow-md shadow-[#00173C]/[0.04] sm:p-6">
        ...
        {session ? (
          <div className="mt-6 rounded-2xl border border-[#DCE7F3] bg-[#F8FAFC] p-4">
            ...
            {questionProviderMessage ? (
              <p className="mt-4 rounded-xl border border-[#DCE7F3] bg-white p-3 text-sm font-semibold text-[#00173C]">
                {questionProviderMessage}
              </p>
            ) : null}
          </div>
